...
namespace Antivaly.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            else if (Session["AccType"].ToString() == "Seller")
            {
        ...
        public ActionResult Index(LoginValidation user)
        {
            if (ModelState.IsValid)
            {
                ...
                using (AntivalyEntities db = new AntivalyEntities())
                {
        ...
        public ActionResult Registration()
        {
            ...
            ViewBag.SellerCount = db.UserIDs.FirstOrDefault(u => u.ID == SID).LastID;
        ...
        public static void UserIDChange(RegistrationValidation user)
        {
            ...
            {
                var db = new AntivalyEntities();
                var oldUserID = db.UserIDs.FirstOrDefault(u => u.ID == 3);
                db.Entry(oldUserID).CurrentValues.SetValues(new UserID()
                {
                    ID = oldUserID.ID,
                    AccType = oldUserID.AccType,
